package com.liujun.datastruct.base.datastruct.heap.solution.margeSmailFile;

import com.liujun.datastruct.base.datastruct.heap.solution.margeSmailFile.pojo.FileMargeBusi;
import com.liujun.datastruct.base.datastruct.heap.solution.margeSmailFile.pojo.ByteHeadInfo;
import com.liujun.datastruct.base.datastruct.heap.solution.margeSmailFile.pojo.OutFileBusi;
...
public class MargeProc {
  ...
  public void reader(FileMargeBusi[] margeFile, String outPath) {
    ...
    // 如果当前文件未结束，则继续遍历,这边遍历，直到所有的文件都读取完
    while (!checkFinish(margeFile) || !smallHeap.isEmpty()) {
      ...
      // 将当前文件的下一个数据加入到当前小顶堆中,也就是从堆顶元素的同一个文件中获取
